$(function () {
  ...
  function updateEmployeeTable(employees) {
    ...
    employees.forEach(function (employee, index) {
      ...
      $table.append(
        `<tr>
                <th scope="row" class="w-10"><input class="form-check-input" type="checkbox" value="${
                  employee.id
                }" id="check-input-${employee.id}"></th>
                <td><span class="badge text-bg-primary">${
                  employee.tenure
                }</span></td>
                <td data-bs-toggle="modal" data-bs-target="#employeeDetailsModal" data-id="${
                  employee.id
                }"><span>${employee.name}</span></td>
                <td>${employee.departments}</td>
                <td>${
                  employee.position || '<span class="text-warning">!</span>'
                }</td>
                <td>${
                  employee.hrieDate || '<span class="text-warning">!</span>'
                }</td>
                <td>${employee.moneyMaybe || '-'}</td>
                <td>${
                  employee.moneyGood
                    ? numberFormatter.format(employee.moneyGood)
                    : '<span class="text-warning" style="margin-left: 35px;">!</span>'
                }</td>
                    <td>${
                      employee.moneyBye
                        ? numberFormatter.format(employee.moneyBye)
                        : '<span class="text-warning" style="margin-left: 35px;">!</span>'
                    }</td>
                    <td>${
                      employee.realMoney
                        ? numberFormatter.format(employee.realMoney)
                        : '<span class="text-warning" style="margin-left: 59px;">!</span>'
                    }</td>
                    <td>${
                      employee.realMoney
                        ? numberFormatter.format(employee.realMoney)
                        : '<span class="text-warning" style="margin-left: 45px;">!</span>'
                    }</td>
            </tr>`
      );
    });
    ...
    for (let R = range.s.r; R <= range.e.r; R++) {
      ...
      for (let C = range.s.c; C <= range.e.c; C++) {
        ...
        if (
          ...
          cellValue = `${dateValue.y}-${String(dateValue.m).padStart(
            2,
            '0'
          )}-${String(dateValue.d).padStart(2, '0')}`;
        } else if ([6, 7, 8].includes(C) && typeof cellValue === 'string') {
          // 금액 필드
        ...
        switch (C) {
          case 0:
            employee.tenure = cellValue;
            break;
          case 1:
            employee.name = cellValue;
            break;
          case 2:
            employee.departments = cellValue;
            break;
          case 3:
            employee.position = cellValue;
            break;
          case 4:
            employee.hrieDate = cellValue;
            break;
          case 5:
            employee.moneyMaybe = cellValue;
            break;
          case 6:
            employee.moneyGood = cellValue;
            break;
          case 7:
            employee.moneyBye = cellValue;
            break;
          case 8:
            employee.realMoney = cellValue;
            break;
        }
      }
      ...
      if (filteredData.length) {
        filteredData.forEach((employee) => {
          resultsContainer.append(
            `<div class="autocomplete-item">${employee.name}</div>`
          );
        });
  ...
  $('.btn-search').on('click', function () {
    ...
    var filteredEmployees = allEmployees.filter((employee) =>
